...
class VictoryConditionTests {
    ...
    fun having3x3Field_onlyOnePlayerMarksAreSet_victoryConditionIsCorrect() {
        ...
        game.makeMove(game.gameField.getCoordinatesFor(0, 0), playerX)
        game.makeMove(game.gameField.getCoordinatesFor(1, 0), playerX)
        game.makeMove(game.gameField.getCoordinatesFor(2, 0), playerX)
    ...
    fun having2LinesOfOnePlayerOn5x5Field_thisPlayerMarkIsSetInBetween_victoryConditionIsCorrect() {
        ...
        game.makeMove(game.gameField.getCoordinatesFor(3, 0), playerX)
        game.makeMove(game.gameField.getCoordinatesFor(4, 0), playerX)
    ...
    fun test3x3FieldWithMultiplePossibleLines() { // test name is left as it was in the pull-request
        ...
        game.makeMove(game.gameField.getCoordinatesFor(1, 1), playerX)
        game.makeMove(game.gameField.getCoordinatesFor(2, 1), playerO)
        game.makeMove(game.gameField.getCoordinatesFor(2, 0), playerX)
    ...
    fun having3x3Field_realSimulation2PlayersMovesMade_victoryConditionIsCorrect() {
        val game = Game.create() as GameSession
        game.makeMove(game.gameField.getCoordinatesFor(0, 0)) // X
        game.makeMove(game.gameField.getCoordinatesFor(1, 0)) // O
        game.makeMove(game.gameField.getCoordinatesFor(2, 0)) // X
        game.makeMove(game.gameField.getCoordinatesFor(1, 1)) // O
        game.makeMove(game.gameField.getCoordinatesFor(2, 1)) // X
        game.makeMove(game.gameField.getCoordinatesFor(1, 2)) // O - > O wins here
        /*
            X O X
            . O X
            . O .
